...
public class LocalPhotoActivity extends FragmentActivity implements
	...
	private void showPhotoPickerFragment(String albumName, List<PhotoInfo> list) {
		...
		if (!photoPickerFragment.isAdded()) { // 先判断是否被add过,如果没有add过,直接add这个fragment;
			...
			transaction.hide(photoFolderFragment)
					.add(R.id.fragment_container, photoPickerFragment).commit(); // 隐藏当前的fragment，add下一个到Activity中
		} else { // 如果已经add过,隐藏这个fragment,并且显示下一个fragment;
			photoPickerFragment.updateDataList(list);
			transaction.hide(photoFolderFragment).show(photoPickerFragment)
	...
	private void showPhotoAlbumFragment() {
		...
		if (!photoFolderFragment.isAdded()) {
			transaction.hide(photoPickerFragment)
					.add(R.id.fragment_container, photoFolderFragment).commit();
		} else {
			transaction.hide(photoPickerFragment).show(photoFolderFragment)
					.commit();
		}
	...
	/*
	 * 在照片选择界面选择了照片集后回调该方法; fragment将已经选择的照片列表回传;
	 */
	@Override
	...
	public void finish() {
		super.finish();
		this.overridePendingTransition(R.anim.activity_stay,
				R.anim.photo_picker_activity_close);
	}
